...
void Timer::Start() {
  if (!running()) {
    if (Caffe::mode() == Caffe::GPU) {
      ...
      start_cpu_ = std::chrono::system_clock::now();
    }
    running_ = true;
...
void Timer::Stop() {
  if (running()) {
    if (Caffe::mode() == Caffe::GPU) {
      ...
      stop_cpu_ = std::chrono::system_clock::now();
    }
    running_ = false;
  }
}
...
void CPUTimer::Start() {
  if (!running()) {
    ...
    this->start_cpu_ = std::chrono::system_clock::now();
    this->running_ = true;
...
void CPUTimer::Stop() {
  if (running()) {
    ...
    this->stop_cpu_ = std::chrono::system_clock::now();
    this->running_ = false;
  }
}
...
float CPUTimer::MilliSeconds() {
  ...
  this->elapsed_milliseconds_ = std::chrono::duration_cast<std::chrono::milliseconds>(this->stop_cpu_-this->start_cpu_).count();
